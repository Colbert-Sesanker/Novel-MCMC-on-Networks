# LICENSE

Copyright (c) 2012-2016 Colbert Sesanker

This work is licensed under the **Creative Commons Attribution-NonCommercial 4.0 International License (CC BY-NC 4.0)**.

### You are free to:
- **Share:** Copy and redistribute the material in any medium or format.
- **Adapt:** Remix, transform, and build upon the material.

### Under the following terms:
- **Attribution:** You must give appropriate credit, provide a link to the license, and indicate if changes were made (e.g., files, functions). You may do so in any reasonable manner, but not in any way that suggests the licensor endorses you or your use.
- **Non-Commercial:** You may not use the material for commercial purposes or any financial gain.

To view a copy of this license, visit:  
https://creativecommons.org/licenses/by-nc/4.0/

`SPDX-License-Identifier: CC-BY-NC-4.0`

---

## Third-Party Code Notices

1. **"Period Control of Biological Oscillators"**  
   Includes heavily modified and rewritten code in:

       "Period Control of Biological Oscillators"
       └── "ReactionNetworks (Overrides SloppyCell Objects)"

   From "SloppyCell":
   - **a)** Christopher R. Myers, Ryan N. Gutenkunst, James P. Sethna  
     *"Python unleashed on systems biology"*  
     Computing in Science and Engineering 9:34 (2007); arXiv:0704.3259

   - **b)** Ryan N. Gutenkunst, Jordan C. Atlas, Fergal P. Casey, Robert S. Kuczenski, Joshua J. Waterfall, Chris R. Myers, and James P. Sethna. *SloppyCell.* http://sloppycell.sourceforge.net/ (2007)

   The novel objects in "ReactionNetworks (Overrides SloppyCell Objects)" override the default "SloppyCell" objects in the source repository referenced above.

2. A few other third-party function files (e.g., LASSO and cubic splines) are included.  
   For all such files, the license and copyright associated with each file are found in the opening block comment.  
   **All files without an explicit license in the comment header were written by Colbert Sesanker.**

3. **All code in "MCMC/MCMC (MATLAB)" was written by Colbert Sesanker**, based on theory from:

   Girolami, M. and Calderhead, B. (2011), Riemann manifold Langevin and Hamiltonian Monte Carlo methods.  
   Journal of the Royal Statistical Society: Series B (Statistical Methodology), 73: 123–214.  
   doi: 10.1111/j.1467-9868.2010.00765.x
